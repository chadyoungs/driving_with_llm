...
def nextFramePrompts(
    ...
    if nextFrame <= maxFrame:
        ...
        gr.Error("The range of Decision Frame is {}~{}.".format(minFrame, maxFrame))


def lastFramePrompts(
    decisionFrame: int,
):
...
if __name__ == "__main__":
    ...
    with gr.Blocks(theme=gr.themes.Base(text_size=gr.themes.sizes.text_lg)) as demo:
        with gr.Row(
            ...
            lastFrameBtn = gr.Button(
                scale=1,
                value="Last Frame",
            )
            nextFrameBtn = gr.Button(
                scale=1,
                value="Next Frame",
            )
