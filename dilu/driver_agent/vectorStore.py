    ...
    def __init__(self, encode_type='sce_language', db_path=None) -> None:
        ...
        if encode_type == 'sce_encode':
            ...
            if os.environ["OPENAI_API_TYPE"] == 'azure':
                ...
                self.embedding = OpenAIEmbeddings()
            elif os.environ["OPENAI_API_TYPE"] == 'open_ai':
                self.embedding = OpenAIEmbeddings(base_url="http://localhost:1234/v1", 
                                                  api_key=os.getenv("LOCAL_MODEL_NAME"), model=os.getenv("LOCAL_MODEL_NAME"))
            else:
                raise ValueError(
                    "Unknown OPENAI_API_TYPE: should be azure, openai, or open_ai")
