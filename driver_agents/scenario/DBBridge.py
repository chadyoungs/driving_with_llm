...
<<<<<<<< HEAD:driving_with_llm/scenario/DBBridge.py
from highway_env.envs.common.abstract import AbstractEnv
from highway_env.road.road import RoadNetwork, LaneIndex
from highway_env.road.lane import StraightLane, CircularLane, SineLane
...
>>>>>>>> dl_develop:driver_agents/scenario/DBBridge.py
...
<<<<<<<< HEAD:driving_with_llm/scenario/DBBridge.py
    ...
    def getSineLaneWayPoint(self, cl: SineLane):
        theta = np.linspace(0, angle_range, num=50)
        x = cl.start[0] + radius * np.sin(theta)
        y = cl.start[1] + radius * (1 - np.cos(theta))
        return ' '.join([f'{x[i]},{y[i]}' for i in range(len(x))])
...
>>>>>>>> dl_develop:driver_agents/scenario/DBBridge.py
                        ...
                        wayPoint = self.getSineLaneWayPoint(lane)
                        cur.execute(
                            """INSERT INTO networkINFO(
                                laneIndexO, laneIndexD, laneIndexI, laneType, 
                                wayPoint, width, speedLimit
    ...
    def insertVehicle(
        ...
        cur.execute(
            ...
            (
                ...
                self.ego.position[1],
                self.ego.speed,
                self.ego.action["acceleration"],
                self.ego.heading,
                self.ego.action["steering"],
                ek1,
                ek2,
                ek3,
            ),
